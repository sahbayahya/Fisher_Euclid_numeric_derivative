PROGRAM Fisher_Distance
 ...
 double precision :: wdamp_perp, wdamp_para, wdamp_silk, wdamp_factor, wdamp_sinterm
  ...
  double precision :: zmin,zmax,area,Vsurvey,Vtot,Ntot,Ngl, ngal,dndz,da, one_over_h, Vsurvey2, dvdz2, omz, D_plus
  ...
  om0=0.267d0!0.316d0
  ode0= 0.686d0
  ok0 = 0d0!1d0 - om0-ode0
  ob0 = 0.049
  ok0 = 0.d0! 1d0 - om0 -ob0-ode0
  w0=-1.d0
  w_a = 0.d0
...
!==============Enter the File name ================================== 
!list = [0, 3, 7, 23, 70, 100, 200]
!print*,  list
filename = 'input_Fisher_bao_0n_rms.txt'
!filename = 'SKA1MID_B2_Pess_Corrected_nz_Srms.txt'
!filename = 'SKA2_Pess_Corrected_nz_Srms.txt'
!filename = 'SKA1SUR-B1_corrected_nz_Srms.txt'
!print*, filename
!filename='input_Fisher_bao_7_rms_s30k_z=1.txt'!
...
!================================================================  
  ...
  !=========== Loop over redshift bins =======================================
  do ibin=1,100
    ! read(2,*,end=10)z ,dndz,bias,kmax_ov_h,kmin_ov_h,delta_v, Vsurvey,D_plus ! uncomment for Euclid
     read(2,*,end=10) z ,dndz,bias, kmax_ov_h, Vsurvey, dvdz2! Uncomment for SKA
     nbins=nbins+1
     !-------------------------------------------------------------------
     ! to call the volume uncomment the next 4 lines 
     ! area = 15000d0
     !zmin  = z - 0.5d0
     ! zmax = z+ 0.5d0
    !CALL volume(zmin,zmax,area,Vsurvey2)
     !z=0.5d0*(zmin+zmax)
     !--------------------------------------------------------------------
     beta=(1d0+dgdlna(z)/g(z))/bias ! it does match phil's
     w = w0 + w_a*(z/(1d0+z))
     !ngal=dndz  !  Uncomment for  Euclid
     dvdz2 = dvdz2*(H_0/100d0)**3 ! convert to Mpc^3 h^-3 Uncomment for SKA
     ...
     !print*, 'dvdz from volume.f90' , dvdz(z)
     !print*, 'dvdz from fit_Euclid', dvdz2*(H_0/100d0)**3
    ! fz =(1d0 + z)**(3d0*(1d0 + w0+ w_a)) * exp(-3d0 * w_a *(z/(1d0+ z)))
    !    sigma_z=(1d0+z)*(delta_v/2.998d5) &
    !      *(2.998d3/dsqrt((ob0+om0)*(1d0+z)**3d0+ode0)) ! in units of h^-1 Mpc
        !sigma_perp=sigma0*g(z)/(1d0+z) ! in units of h^-1 Mpc
       !sigma_para=sigma0*g(z)/(1d0+z)*(2d0+dgdlna(z)/g(z)) ! in units of h^-1 Mpc
       !==================Test the growth =================================
      !  open(101,file='test_all_terms_SKA1MID_B2_Pess_200_Srms_corrected_nz.txt' ,status='unknown')
      ! write(101,*) z, ngal, Vsurvey, g(z) , beta*bias, bias, beta!, D_plus!/(1d0+z), (1d0+dgdlna(z)/g(z))
       ! write(101,*) z, Vsurvey, dvdz2, dndz, ngal, bias, beta, beta*bias
     ...
     do while (k_ov_h <=kmax_ov_h)
      ...
      ! open(100,file='cosmic_variance_7.txt' ,status='unknown')
    ...
    print'(1A7,1F8.5)','gin/in+1 =',g(zin)/(1d0 +zin)
...
!open(12,file='Fisher_bao_Pess_23_Srms_corrected_nz_2.txt' ,status='unknown')
open(12,file='Fisher_bao_0n_rms_s30k.txt' ,status='unknown')
!open(12,file='Fisher_bao_Euclid_s15k_2.txt' ,status='unknown')
...
! open(13,file='output_Fisher_bao_Euclid_s15k_2.txt', status='unknown')
