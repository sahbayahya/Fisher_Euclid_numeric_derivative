PROGRAM Fisher_Distance
 ...
 double precision :: wdamp_perp, wdamp_para, wdamp_silk, wdamp_factor, wdamp_sinterm
  ...
  double precision :: zmin,zmax,area,Vsurvey,Vtot,Ntot,Ngl, ngal,dndz,da, one_over_h
...
!  do j=1,n1
      ...
      !  print*, k_ov_h
        ...
        do while (mu<=1d0)
         ...
         !  p02=pk_ref *(1d0 + fbao)*(g(z)/g(zin)*(1d0+zin)/(1d0+z))**2d0 &
         ...
         !  print*, k_ov_h ,  wkmu
           wdamp = ((dfbao/(1d0 + fbao))*pk* k_ov_h)**2d0
           der(1)=(mu2-1d0) ! dPkdlogDa
           der(2)=mu2 ! dPkdlogH
        !   print*,  der(1), der(2)
           do i=1,npara
   ...
   !   write(5,*) k_ov_h, pk! , 1d0/ngal
    ...
    !	open(16,file='cosmic_v_k_Euclid_fis_test.txt' ,status='unknown')
...
!open(12,file='Fisher_Euclid_diff_14bins_S3.txt' ,status='unknown')
